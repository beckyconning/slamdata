  ...
  peekDeck (Deck.DoAction (Deck.Unwrap decks) _) = void $ runMaybeT do
    ...
    case error of
      Left err →
        case GE.fromQError err of
          Left msg →
            Notify.error "Failed to collapse deck." (Just $ Notify.SimpleDetail msg) Nothing
          Right ge →
  ...
  peekDeck (Deck.DoAction Deck.Wrap _) = void $ runMaybeT do
    ...
    case error of
      Left err →
        case GE.fromQError err of
          Left msg →
            Notify.error "Failed to wrap deck." (Just $ Notify.SimpleDetail msg) Nothing
          Right ge →
            GE.raiseGlobalError ge
      Right _  → do
  ...
  peekDeck (Deck.DoAction Deck.Mirror _) = void $ runMaybeT do
    ...
    case error of
      Left err →
        case GE.fromQError err of
          Left msg →
            Notify.error "Failed to mirror deck." (Just $ Notify.SimpleDetail msg) Nothing
          Right ge →
            GE.raiseGlobalError ge
      Right _  → do
