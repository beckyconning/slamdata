...
findSingle locator = do
  ...
  case uncons elements of
    Nothing -> throwError $ error $ "Couldn't find an element with the locator: " ++ showLocator locator
...
findAtLeast n locator = do
  ...
  if length elements >= n
    then pure $ elements
    else throwError $ error $ "Couldn't find at least " ++ show n ++ " elements with the locator: " ++ showLocator locator
